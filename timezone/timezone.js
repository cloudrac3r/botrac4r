            ...
            code: function(userID, channelID, command, d) {
                ...
                tzdb.get("SELECT UTCoffset FROM Users WHERE userID = ?", target, function(err, dbr) {
                    if (!dbr) {
                        ...
                        bf.sendMessage(channelID, `The local time of ${bf.userIDToNick(target, bot.channelGuildMap[channelID], "nickname")} is **${cf.getReadableTime(time, 2, true)}**.`, {mention: userID});
                    }
                });
            }
        },
